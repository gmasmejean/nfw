...
jhtml.prototype.createAll = function( configuration, callback ){
    ...
    for( var name in configuration ){
        evtStack++;
        this.createTemplateModule(configuration[name],name,cb);
    }
};

jhtml.prototype.createTemplateModule = function( path, name, callback){
    fs.exists( path+'/'+name+'.tpl.js',function(exist){
        if(!exist){
            fs.exists( path+this.jhtmlPath+'/'+name+'.jhtml',function(exist){
                if( !exist )
                ...
                fs.writeFileSync(path+'/'+name+'.jtpl.js',fn,{encoding:'utf8'});
